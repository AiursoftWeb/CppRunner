...
1. Navigate to the MySQL project directory:
   ```bash
   cd ./src/Aiursoft.CppRunner.MySql/
   ```
...
```bash
dotnet ef migrations remove --context "MySqlContext" -s ../Aiursoft.CppRunner/Aiursoft.CppRunner.csproj
```

## After Creating Migrations

After creating migrations for MySQL, remember to:
1. Create the corresponding migration for SQLite (see `../Aiursoft.CppRunner.Sqlite/HOW_TO_ADD_MIGRATIONS.md`)
